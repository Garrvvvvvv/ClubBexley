...
export default function MyRegistrations() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-20 px-4 md:px-8 relative overflow-hidden">
      ...
      {/* DETAILS MODAL */}
      {selectedReg && (
        <div className="fixed inset-0 z-5000 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setSelectedReg(null)}>
                ...
                <div>
                  ...
                  {selectedReg.receipt && (
                    <div className="mt-4 flex justify-between items-center bg-gray-50 p-3 rounded-lg border border-gray-100">
                      <span className="text-sm text-gray-500 font-medium">Payment Receipt</span>
                      <button
                        onClick={async () => {
                          setLoadingReceipt(true);
                          try {
                            const res = await apiUser.get(`/api/events/${selectedReg.event.slug}/me/receipt`, { responseType: 'blob' });
                            setViewReceipt(URL.createObjectURL(res.data));
                          } catch (e) {
                            console.error(e);
                          } finally {
                        ...
                        className="text-[#ca0002] text-sm hover:underline font-bold"
      ...
      {/* RECEIPT MODAL */}
      {viewReceipt && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setViewReceipt(null)}>
