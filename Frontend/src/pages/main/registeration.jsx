...
export default function Registration() {
  ...
  useEffect(() => {
    ...
    const fetchEvent = async () => {
      ...
      try {
        const res = await apiUser.get(`/api/events/${eventSlug}`);
        if (isMounted) setEvent(res.data);
      } catch (err) {
        console.error(err);
        if (isMounted) toast.error("Could not load event details.");
      } finally {
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
              ...
              <div className="flex justify-between items-center">
                ...
                {serverRecord.receipt && (
                  <button
                    onClick={async () => {
                      setLoadingReceipt(true);
                      try {
                        const res = await apiUser.get(`/api/events/${eventSlug}/me/receipt`, { responseType: 'blob' });
